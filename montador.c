...
int swap_bytes(short int valor){
    // Muda posição de bytes dos offsets
    short int resultado;
    resultado = valor >> 8;
...
int existe(char *str, char **lista, int tamanho){
    // Checa se váriavel já está na lista
    for(int i = 0; i < tamanho; i++){
        if(!strcmp(str, *(lista + i)))
            return 1;
    }
...
int retorna_offset(char *label, char **lista_labels, int *lista_offsets, int tamanho){
    // Retorna o offset de uma determinada label
    for(int i = 0; i < tamanho; i++){
...
int main(int argc, char *argv[]){
    if(argc  < 3){
        printf("Uso: %s prog.asm prog.exe\n", argv[0]);
        exit(1);
    }
    ...
    int indice = 0; // indice interno (para variáveis locais); tamanho da lista de variáveis
    int indice_offset = 0; // indice para a lista de offsets; tamanho da lista de offsets
    int offset = 0; // offset da instrução atual
    
    int tamanho_cod_ini = 20;	// tamanho do código em si + 20 bytes de inicialização
    short int init = 0x73;
